...
trait CommandGateway {
  def sendAndAsk[T <: DomainCommand](command: T)(
      implicit validator: ValidateableCommand[T]): Future[_]
  def send[T <: DomainCommand](command: T)(
      implicit validator: ValidateableCommand[T]): Future[Unit]
}
...
class DefaultCommandGateway[A <: AggregateRootCreator](
  ...
  private def getAggregateRoot(c: DomainCommand)(
